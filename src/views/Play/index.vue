...
export default {
  ...
  methods: {
     getSng() { 
      // 获取歌曲详情, 和歌词方法
      getSongByIdApi(this.id).then((res)=>{
        this.songInfo = res.data.songs[0]
        getLyricByIdApi(this.id).then((lyrContent)=>{
    ...
    showLyric() {
      ...
      this.$refs.audio.addEventListener('timeupdate', () => {
        ...
        if (this.lyric[curTime]) {
          this.curLyric = this.lyric[curTime].replace("网易","青柠-音乐");
          this.lastLy = this.curLyric
        } else {
          this.curLyric = this.lastLy.replace("网易","青柠-音乐");
        }
      })
    },
    fncc(){
